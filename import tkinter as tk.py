...
# -----------------------------------------------------------
# YOUR DRONE CLASS (Now includes Speed Logic fixes)
# -----------------------------------------------------------
class Drone:
    ...
    # --- ADDED SPEED LOGIC HERE ---
    def increase_speed(self, kmph):
        if self.battery < 5:
            return "Battery too low to speed up."
        self.speed += kmph
        self.battery -= 0.2 * kmph
        return f"Speed increased by {kmph} km/h."

    def decrease_speed(self, kmph):
        if self.speed == 0:
            return "Drone is stationary."
        actual_change = min(self.speed, kmph)
        self.speed -= actual_change
        self.battery -= 0.2 * actual_change
        return f"Speed decreased by {actual_change} km/h."
    # ------------------------------
...
class DroneControlPanel:
    def __init__(self, root):
        ...
        self.root.geometry("600x550") # Made window slightly taller
        ...
        self.control_frame.place(x=290, y=20, width=280, height=300)
        ...
        # Row 3: Altitude
        tk.Button(self.control_frame, text="Alt Up", command=lambda: self.action_altitude("up")).grid(row=3, column=0, sticky="ew", padx=2, pady=2)
        tk.Button(self.control_frame, text="Alt Down", command=lambda: self.action_altitude("down")).grid(row=3, column=1, sticky="ew", padx=2, pady=2)

        # Row 4: Speed (NEW!)
        tk.Button(self.control_frame, text="Speed +", command=lambda: self.action_speed("up")).grid(row=4, column=0, sticky="ew", padx=2, pady=2)
        tk.Button(self.control_frame, text="Speed -", command=lambda: self.action_speed("down")).grid(row=4, column=1, sticky="ew", padx=2, pady=2)

        # Rows 5-7: Direction Pad
        tk.Button(self.control_frame, text="North", command=lambda: self.action_move("north")).grid(row=5, column=0, columnspan=2, sticky="ew", padx=2, pady=2)
        tk.Button(self.control_frame, text="West", command=lambda: self.action_move("west")).grid(row=6, column=0, sticky="ew", padx=2, pady=2)
        tk.Button(self.control_frame, text="East", command=lambda: self.action_move("east")).grid(row=6, column=1, sticky="ew", padx=2, pady=2)
        tk.Button(self.control_frame, text="South", command=lambda: self.action_move("south")).grid(row=7, column=0, columnspan=2, sticky="ew", padx=2, pady=2)

        # --- BOTTOM: LOGS ---
        tk.Label(root, text="Mission Logs:").place(x=20, y=330)
        self.log_area = scrolledtext.ScrolledText(root, width=68, height=10)
        self.log_area.place(x=20, y=350)
    ...
    # --- NEW SPEED ACTION ---
    def action_speed(self, direction):
        val = self.get_value()
        if direction == "up":
            msg = self.drone.increase_speed(val)
        else:
            msg = self.drone.decrease_speed(val)
        self.update_gui(msg)
