...
class Drone:
    ...
    def gyroscope_sensor(self):
        ...
        self.add_log("Gyroscope readings - Pitch: " + str(pitch) +
                    "°, Roll: " + str(roll) + "°, Yaw: " + str(yaw) + "°.")
    ...
    def apply_wind(self):
        if self.altitude > 0:
            wind_x = random.randint(-2, 2)
            wind_y = random.randint(-2, 2)
            self.x += wind_x
            self.y += wind_y
            print(f"Wind pushed the drone by ({wind_x}, {wind_y}) meters.")
            self.add_log(f"Wind drift: ({wind_x}, {wind_y})")
    ...
    def take_off(self):
        ...
        self.speed = 5
        self.battery -= 5
        print("The drone", self.name, "has taken taken off.")
    ...
    # --------------------------------------
    # AUTO RETURN HOME (FIXED)
    # --------------------------------------
    def auto_return_home(self):
        if self.battery <= 15:
            print("The drone", self.name, "is returning home due to low battery.")
            self.altitude = 0
            self.speed = 0
            self.battery -= 3
            self.x = 0
            ...
            self.add_log("Auto return home (low battery).")
    ...
    def increase_altitude(self, meters):
        ...
        self.add_log(f"Increased altitude by {meters} meters.")
    ...
    def decrease_altitude(self, meters):
        ...
        if meters > self.altitude:
            print("Cannot decrease altitude below 0.")
            return
        self.altitude -= meters
        self.battery -= 0.2 * meters
        print("The drone", self.name, "has decreased altitude by", meters, "meters.")
        self.add_log(f"Decreased altitude by {meters} meters.")
    ...
    def increase_speed(self, kmph):
        if self.battery < 5:
            print("Battery too low to increase speed.")
            return
        self.speed += kmph
        self.battery -= 0.2 * kmph
        print("The drone", self.name, "has increased speed by", kmph, "km/h.")
        self.add_log(f"Increased speed by {kmph} km/h.")
    ...
    def decrease_speed(self, kmph):
        ...
        self.add_log(f"Decreased speed by {kmph} km/h.")
    ...
    def move_north(self, meters):
        ...
        self.add_log(f"Moved north by {meters} meters.")
        self.apply_wind()
    ...
    def move_south(self, meters):
        ...
        self.add_log(f"Moved south by {meters} meters.")
        self.apply_wind()
    ...
    def move_east(self, meters):
        ...
        self.add_log(f"Moved east by {meters} meters.")
        self.apply_wind()
    ...
    def move_west(self, meters):
        ...
        self.add_log(f"Moved west by {meters} meters.")
        self.apply_wind()
